return {
  ...
  opts = require("user.plugins.diffview.config"),
  config = function(_, opts)
    -- obtained from
    --   https://code.mehalter.com/AstroNvim_user/~files/5eaf34776def650fc305823ba9bee8460c5eee83/plugins/diffview.lua?position=source-4.1-120.7-1
    ...
    utils.set_mappings({
      n = {
        ...
        -- [prefix .. "q"] = { "<cmd>DiffviewClose<cr>", desc = "Close DiffView" },
      },
    })

    -- add new keymaps with the default keymaps
    -- only when DiffView is opened
    local build_keymaps = function(maps)
      ...
      do
        ...
        out[i] = { mode, lhs, rhs, keymap_opts }
        i = i + 1
      end
      return out
    end
